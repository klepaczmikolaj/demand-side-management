...
import pl.wut.wsd.dsm.agent.customer_handler.model.Identifiable;
...
public class HibernateTemplate {
    ...
    public HibernateTemplate(final String url, final String user, final String password) {
        ...
        settings.setProperty(Environment.DRIVER, Driver.class.getCanonicalName());
        ...
        settings.setProperty(Environment.DIALECT, MySQL8Dialect.class.getCanonicalName());
        ...
        sources.addAnnotatedClass(Obligation.class);
    ...
    public <T> Optional<T> findOne(final Specification<T> specification, final Class<T> clazz) {
        return fetchInTransaction(session -> {
            final Query<T> query = buildQuery(specification, clazz, session);
            try {
    ...
    private <T> Query<T> buildQuery(final Specification<T> specification, final Class<T> clazz, final Session session) {
